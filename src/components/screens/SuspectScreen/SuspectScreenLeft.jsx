...
import {
	getGeneralDetailsFromStory,
	getSuspectListFromStory,
} from "../../../utils/TabList";
...
const SuspectScreenLeft = ({ onSelect, onInterrogationStart }) => {
	...
	const generalDetails = getGeneralDetailsFromStory(currentStory);
	...
	useEffect(() => {
		setSelectedTabId(generalDetails.Id);
	...
	return (
		<div className="w-full md:w-1/3 lg:w-1/4 bg-black/50 backdrop-blur-sm border border-gray-700 rounded-lg p-6 flex flex-col h-full max-h-[90vh]">
			...
			<button
				onClick={() => handleSelect(generalDetails)}
